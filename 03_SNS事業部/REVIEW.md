...
## 2026-07-29 衣装ローテーション監査 Review

- `WARDROBE_ROTATION_REPORT.md`を生成できる。
- `TODAY_POST.md`へ衣装タグと衣装根拠を表示できる。
- 今日の素材は`prompt.txt`/`report.md`が未取得のため、衣装タグは推測せず`未取得`として記録。
- `fail/`配下の画像を採用候補件数から除外済み。
- SNS投稿、画像生成、ファイル削除は未実行。

## 残課題

- 生成時に`prompt.txt`へ衣装テーマを必ず保存する。
- `report.md`へ優先候補と衣装メモを必ず保存する。
