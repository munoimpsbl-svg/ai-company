...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create P003 WordPress draft posts from ARTICLE_QUEUE.md")
    parser.add_argument("--dry-run", action="store_true", help="Validate queue without creating drafts")
    args = parser.parse_args()

    load_env(WORKSPACE_ROOT / ".env")
    try:
        client = WordPressWriteClient.from_env("P003_WORDPRESS_URL")
        client.verify_auth()
